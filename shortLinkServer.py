...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type','text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write('Please Submit it again'.encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type','text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("This link is not existence".encode())
...
(uri, timeout=5):
    ...
    '''
    try:
        return requests.get(uri,timeout=timeout).status_code === requests.codes.ok
